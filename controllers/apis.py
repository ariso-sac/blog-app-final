...
def create_Post(title, text, likes, created_by,comments="Very nice post"):
    d = Posts(title=title, text=text, likes=likes, created_by=created_by, comments=comments)
    db.session.add(d)
    db.session.commit()
    return d
...
class Posts_Create(Resource):
    ...
    def post(self):
        if request.content_type == "application/json":
            ...
            if not (valid([title, text, likes, created_by])):
                return {"Error": "Incorrect Parameter"}, 400
            else:
                d = create_Post(title, text, likes, created_by)
